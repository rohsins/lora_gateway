...
// wrote to solve incomplete shit issue, not in previous file
struct timespece {
	time_t tv_sec;
	long tv_nsec;
};
...
struct tref {
    ...
    struct timespece gps_time;   /*!> reference GPS time (since 01.01.1970) */
...
enum gps_msg {
	...
	/* uBlox proprietary messages of interest */
...
@param gps_familly parameter (eg. ubx7 for uBlox gen.7)
...
@param ps_time pointer to store GPS time, with ns precision (NULL to ignore)
...
/**
@brief Take a timestamp and GPS time and refresh reference for time conversion
